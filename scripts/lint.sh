...
readonly LINT_VERSION="1.58.2"
NEED_INSTALL=false
if command -v golangci-lint >/dev/null 2>&1; then
  # Example: golangci-lint has version 1.58.2 built with go1.22.x
...
run env GOTOOLCHAIN=go1.22.6 goimports -w -l .

while read -r module; do
  run cd ./internal/"$module"
  run env GOTOOLCHAIN=go1.22.6 golangci-lint run --config "$ROOT_DIR/.golangci.yaml"
  run cd -
done < <(modules)
